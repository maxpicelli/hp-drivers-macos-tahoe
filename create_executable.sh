...
# Script para criar um executável portável do instalador de drivers HP
# Este script cria um arquivo executável que pode ser movido para qualquer lugar

echo "=== Criador de Executável HP Drivers ==="
echo ""
...
# Nome do executável
EXECUTABLE_NAME="HP_Drivers_Installer"

echo "Criando executável: $EXECUTABLE_NAME"
...
# HP Drivers Installer - Executável Portável
# Versão: 3.4.0
...
# Verificar se os arquivos estão presentes no diretório do executável
if [ ! -d "$EXECUTABLE_DIR/Library" ] || [ ! -d "$EXECUTABLE_DIR/usr" ]; then
    echo "ERRO: Arquivos dos drivers não encontrados"
    echo "Certifique-se de que este executável está na pasta com os drivers HP"
...
if [ -d "$EXECUTABLE_DIR/Library/Extensions/hp_io_enabler_compound.kext" ]; then
    copy_with_backup "$EXECUTABLE_DIR/Library/Extensions/hp_io_enabler_compound.kext" "/Library/Extensions/"
...
if [ -d "$EXECUTABLE_DIR/Library/Printers/hp" ]; then
    copy_with_backup "$EXECUTABLE_DIR/Library/Printers/hp" "/Library/Printers/"
...
if [ -d "$EXECUTABLE_DIR/Library/Image Capture/Devices" ]; then
    copy_with_backup "$EXECUTABLE_DIR/Library/Image Capture/Devices" "/Library/Image Capture/"
...
if [ -d "$EXECUTABLE_DIR/usr/libexec/cups/backend" ]; then
    for backend in "$EXECUTABLE_DIR/usr/libexec/cups/backend"/*; do
        if [ -f "$backend" ]; then
...
echo "✓ Executável criado com sucesso: $EXECUTABLE_NAME"
echo ""
echo "Como usar:"
echo "1. Mova o executável para onde quiser"
echo "2. Execute: sudo ./$EXECUTABLE_NAME"
echo ""
echo "O executável detectará automaticamente onde está localizado"
echo "e encontrará os arquivos de drivers no mesmo diretório."
