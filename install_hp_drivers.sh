...
# Script que instala drivers HP diretamente nas pastas do sistema
...
if [ "$EUID" -ne 0 ]; then
    ...
    echo "Execute: sudo ./install_hp_drivers.sh"
    exit 1
fi

# Verificar se os arquivos estão presentes no diretório do script
if [ ! -d "$SCRIPT_DIR/Library" ] || [ ! -d "$SCRIPT_DIR/usr" ]; then
    echo "ERRO: Arquivos dos drivers não encontrados"
    echo "Certifique-se de que este script está na pasta com os drivers HP"
...
if [ -d "$SCRIPT_DIR/Library/Extensions/hp_io_enabler_compound.kext" ]; then
    copy_with_backup "$SCRIPT_DIR/Library/Extensions/hp_io_enabler_compound.kext" "/Library/Extensions/"
...
if [ -d "$SCRIPT_DIR/Library/Printers/hp" ]; then
    copy_with_backup "$SCRIPT_DIR/Library/Printers/hp" "/Library/Printers/"
...
if [ -d "$SCRIPT_DIR/Library/Image Capture/Devices" ]; then
    copy_with_backup "$SCRIPT_DIR/Library/Image Capture/Devices" "/Library/Image Capture/"
...
if [ -d "$SCRIPT_DIR/usr/libexec/cups/backend" ]; then
    for backend in "$SCRIPT_DIR/usr/libexec/cups/backend"/*; do
        if [ -f "$backend" ]; then
...
echo "Para desinstalar, execute: sudo ./uninstall_hp_drivers.sh"
